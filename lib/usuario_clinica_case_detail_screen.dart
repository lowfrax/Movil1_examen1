...
class UsuarioClinicaCaseDetailScreen extends StatefulWidget {
  final int idCaso;
  const UsuarioClinicaCaseDetailScreen({super.key, required this.idCaso});

  @override
  State<UsuarioClinicaCaseDetailScreen> createState() => _UsuarioClinicaCaseDetailScreenState();
}

class _UsuarioClinicaCaseDetailScreenState extends State<UsuarioClinicaCaseDetailScreen> {
  ...
  void _setupBleListener() {
    ...
    fbp.FlutterBluePlus.scanResults.listen((results) {
      if (mounted && _isBleScanning) {
        setState(() {
          _bleDevices = results.toSet().toList(); // Filtra duplicados
        });
      }
    });
  }
  ...
  Future<void> _connectToDevice(fbp.BluetoothDevice device) async {
    try {
      ...
      // Buscar la característica TX para recibir datos
      _txCharacteristic = await BleController.findTxCharacteristic(device);
      
      if (_txCharacteristic != null) {
        // Suscribirse a notificaciones
        await _txCharacteristic!.setNotifyValue(true);
        _bleSubscription = _txCharacteristic!.onValueReceived.listen((value) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('No se encontró la característica de datos')),
          );
        }
  ...
  void _handleBleData(List<int> data) {
    try {
      ...
      if (pulseValue != null && pulseValue > 0) {
        setState(() {
          _currentPulse = pulseValue.toString();
  ...
  Future<void> _showBleDialog() async {
    await showDialog(
      ...
      builder: (context) => _BleDialog(
        onConnect: _connectToDevice,
        devices: _bleDevices,
        isScanning: _isBleScanning,
        onStartScan: _startBleScan,
        ...
        _loading
            ...
            : SingleChildScrollView(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  // Botón para emparejar ESP32
                  Container(
                    padding: const EdgeInsets.all(16),
                    decoration: p3PanelDecoration(context),
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      const Text('Sensor de Pulso ESP32', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      const SizedBox(height: 12),
                      if (_isBleConnected) ...[
                        Container(
                          padding: const EdgeInsets.all(16),
...
// Diálogo para escanear y conectar dispositivos BLE
class _BleDialog extends StatelessWidget {
  final Function(fbp.BluetoothDevice) onConnect;
  final List<fbp.ScanResult> devices;
